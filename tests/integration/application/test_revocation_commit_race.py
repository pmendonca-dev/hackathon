...
def test_revocation_after_commit_does_not_cancel_inflight_settlement_but_blocks_next_attempt(tmp_path):
    ...
    mandate = Mandate("m1", Principal("p1", "Marta"), frozenset({"merchant"}), frozenset({"travel"}), Money(1_000, "BRL", 2), datetime(2026, 8, 30, tzinfo=UTC), 1, {"revocation_id": "r1", "epoch": 0}, (RevocationAuthority("a1", "holder", RevocationRole.HOLDER, custody.public_jwk("holder"), frozenset({"mandate"})),), instrument=PaymentInstrument("vt_test_instrument", "•••• 4242"))
    ...
    capture = CaptureCommand("m1", "checkout", "merchant", Money(500, "BRL", 2), "travel", "first", instrument_id="vt_test_instrument")
    ...
    next_result = AuthorizationCore(clock=lambda: now, engine=engine, settlement_adapter=settlement).capture(CaptureCommand("m1", "checkout_2", "merchant", Money(100, "BRL", 2), "travel", "next", instrument_id="vt_test_instrument"))

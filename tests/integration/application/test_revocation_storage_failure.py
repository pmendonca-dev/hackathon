...
def test_capture_fails_closed_when_primary_revocation_storage_is_unavailable(monkeypatch):
    mandate = Mandate(
    ...
    monkeypatch.setattr(SqliteRevocationRepository, "is_revoked", lambda *_args: (_ for _ in ()).throw(OSError("down")))

    result = core.capture(CaptureCommand("m1", "checkout", "merchant", Money(500, "BRL", 2), "idem"))

    assert result.approved is False
    assert result.reason_code == "revocation_unavailable"
